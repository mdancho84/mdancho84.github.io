...
``` {r orderDistMonthYear, echo = FALSE}
# Distribution of orders by month and year
library(dplyr) 
library(ggplot2)
library(lubridate) # For year(), month() functions

# Manipulate the data
ordersByTime <- orders %>%
  mutate(month = month(order.date), 
         month.name = months(order.date),
         year = year(order.date)) %>%
  group_by(year, month, month.name) %>%
  summarise(counts = n()) 

ordersByTime$month.name <- factor(ordersByTime$month.name, 
                                  levels= arrange(ordersByTime, desc(month))$month.name)

# Plot the orders
# ggplot(ordersByTime, aes(x=factor(month), y=counts, color=factor(year))) +
#   geom_point(aes(size=counts)) + 
#   geom_bar(stat="identity", aes(fill=factor(year), width=0.3)) + 
#   facet_wrap(~year) +
#   ylim(c(0,600)) +
#   ggtitle("Distribution of Orders By Month and Year") +
#   xlab("Month") +
#   ylab("Quantity of Orders")

# Plot heat map
ggplot(ordersByTime, aes(x=month.name, y=factor(year))) +
  geom_tile(aes(fill = counts), colour="white") +
  scale_fill_gradient(low = "orange", high = "blue") +
  ggtitle("Heatmap of Order Frequency") +
  xlab("") +
  ylab("") +
...
Using a `rate` = 0.8 still assigns a wide range of orders to customers, and we can see the distribution of order rows by customer in the plot below. Customer `r which.max(table(orders$customer.id))` has the most order rows at `r max(table(orders$customer.id))`, while Customer `r which.min(table(orders$customer.id))` has the least order rows at `r min(table(orders$customer.id))`.

``` {r customerDistribution, echo = FALSE}
# Distribution of orders by customer
custDist <- orders %>% 
  group_by(customer.id) %>% 
  summarize(counts = n()) %>%
  arrange(desc(counts)) %>%
  mutate(customer.id = factor(customer.id))

# Order customer.id 
custDist$customer.id <- factor(custDist$customer.id, levels= arrange(custDist, desc(counts))$customer.id)

ggplot(data=custDist, aes(x=customer.id, y=counts)) + 
  geom_bar(stat="identity") +
  ggtitle("Distribution of Orders Assigned to Customers") +
  ylab("Quantity of Orders")
```
