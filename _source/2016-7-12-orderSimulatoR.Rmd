...
We can see the distribution of order rows by customer using the `table` function. Using a `rate` of 0.8 still assigns a large range of customer lines. Customer `r which.max(table(orders$customer.id))` has the largest number of lines at `r max(table(orders$customer.id))`, while Customer `r which.min(table(orders$customer.id))` has the smallest number of lines at `r min(table(orders$customer.id))`.
...
Nice. Cannondale has a general growth trend. It looks like `r salesByYear[which.max(salesByYear$total.sales), 1]` was Cannondale's best year with `r scales::dollar(max(salesByYear$total.sales))` in total sales, and 2015 almost matched it! Next, let's explore some products to get an idea of top sellers.
