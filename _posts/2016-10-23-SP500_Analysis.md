...
## List of 2
##  $ src    : chr "yahoo"
##  $ updated: POSIXct[1:1], format: "2016-10-29 08:38:50"
{% endhighlight %}
...
It's the stock prices for the first stock, [3M](https://finance.yahoo.com/quote/MMM?p=MMM): all 2470 observations. This is great! We can sort, filter, and perform calculations on all the stock prices at once with the nested tibbles, and the sub-data is all stored in one data frame. 
...
##            <chr>            <dbl>          <dbl>        <dbl>
## 1            MMM     0.0008057663     0.01418006         2470
## 2            ABT     0.0002587388     0.01315323         2470
## 3           ABBV     0.0013412730     0.01705085          960
## 4            ACN     0.0007628010     0.01671923         2470
## 5           ATVI     0.0008639289     0.02226592         2470
## 6            AYI     0.0008792982     0.02378624         2470
## 7           ADBE     0.0006388874     0.02146303         2470
## 8            AAP     0.0004561917     0.02013329         2470
## 9            AES     0.0007084241     0.02511122         2470
## 10           AET     0.0002417444     0.02216750         2470
## # ... with 494 more rows
{% endhighlight %}
...
##    ticker.symbol mean.log.returns sd.log.returns n.trade.days
##            <chr>            <dbl>          <dbl>        <dbl>
## 1            HPE      0.002215791     0.02468773          256
## 2            CFG      0.001874426     0.01716659          525
## 3            MPC      0.001696805     0.02355358         1342
## 4            LYB      0.001690641     0.02220123         1635
## 5            TSN      0.001543213     0.02296755         2470
## 6           AVGO      0.001517202     0.02232179         1817
## 7            HCN      0.001428712     0.01989770         2470
## 8           WLTW      0.001393924     0.01447935          202
## 9            MNK      0.001358715     0.02759503          846
## 10          FBHS      0.001352652     0.01868111         1284
## 11          ABBV      0.001341273     0.01705085          960
## 12           DLR      0.001332034     0.02375196         2470
## 13            HP      0.001237509     0.02956557         2470
{% endhighlight %}
...
##            <chr> <int>            <dbl>          <dbl>
## 1            MPC     1      0.001696805     0.02355358
## 2            LYB     2      0.001690641     0.02220123
## 3            TSN     3      0.001543213     0.02296755
## 4           AVGO     4      0.001517202     0.02232179
## 5            HCN     5      0.001428712     0.01989770
## 6           FBHS     6      0.001352652     0.01868111
## 7            DLR     7      0.001332034     0.02375196
## 8             HP     8      0.001237509     0.02956557
## 9            VTR     9      0.001167633     0.02489993
## 10            MO    10      0.001149183     0.01228646
...
##            <chr>     <date>         <dbl>
## 1            MPC 2011-06-24  0.0000000000
## 2            MPC 2011-06-27  0.0106810937
## 3            MPC 2011-06-28  0.0275066015
## 4            MPC 2011-06-29  0.0347883233
## 5            MPC 2011-06-30  0.0326529638
## 6            MPC 2011-07-01  0.0191393280
## 7            MPC 2011-07-05  0.0002369096
## 8            MPC 2011-07-06 -0.0076100094
## 9            MPC 2011-07-07 -0.0026293820
## 10           MPC 2011-07-08 -0.0088953592
## # ... with 67,375 more rows
{% endhighlight %}

The unnested tibble now contains the daily log returns for the 30 stocks. Notice that there are now 67,385 rows.
...
{% highlight text %}
## # A tibble: 1,115 × 31
##          Date          AAPL          AIZ          ALB          ALK
##        <date>         <dbl>        <dbl>        <dbl>        <dbl>
## 1  2012-05-18  0.0004903026 -0.029894742 -0.011401872  0.004338335
## 2  2012-05-21  0.0566261244  0.019658041  0.039512534  0.012903399
## 3  2012-05-22 -0.0077084148 -0.045514069 -0.034969745 -0.009507738
## 4  2012-05-23  0.0241068595 -0.014954168  0.008857730 -0.006802827
## 5  2012-05-24 -0.0092263231  0.006518627  0.006468275  0.048745879
## 6  2012-05-25 -0.0053742562 -0.018179643  0.015420234  0.003539790
## 7  2012-05-29  0.0175931559  0.006594802  0.035972226  0.004993452
## 8  2012-05-30  0.0119851837 -0.010210313 -0.046609785 -0.015055725
## 9  2012-05-31 -0.0024893776  0.007517689 -0.001317052  0.020024244
## 10 2012-06-01 -0.0294036247 -0.016005244 -0.064648351 -0.014388670
## # ... with 1,105 more rows, and 26 more variables: AVGO <dbl>,
## #   AWK <dbl>, CMS <dbl>, CSX <dbl>, CTAS <dbl>, DLPH <dbl>,
## #   DLR <dbl>, FB <dbl>, FBHS <dbl>, FISV <dbl>, GPN <dbl>,
## #   HCN <dbl>, HCP <dbl>, HP <dbl>, HRS <dbl>, IR <dbl>, LYB <dbl>,
## #   MO <dbl>, MPC <dbl>, O <dbl>, PRGO <dbl>, SWKS <dbl>, TSN <dbl>,
## #   UA <dbl>, VTR <dbl>, WMB <dbl>
{% endhighlight %}

Finally, we are ready to get and visualize correlations. We'll use the `corrplot` package for visualizing the correlations. We remove the `Date` column so only values remain. Then, we send to the `cor()` function, which calculates the correlations. Finally, we send to `corrplot()` to visualize the correlations. I ordered using `hclust`, which groups the stocks by similarity. The `addrect` argument adds boxes around the highly correlated groups, and by inspecting the circle colors I settled on six boxes. 
...
{% highlight text %}
##           AAPL       AIZ       ALB       ALK      AVGO       AWK
## AAPL 1.0000000 0.2093468 0.2992248 0.2510943 0.4152308 0.1361769
## AIZ  0.2093468 1.0000000 0.3575804 0.3163156 0.3057328 0.2657427
## ALB  0.2992248 0.3575804 1.0000000 0.2575325 0.3943849 0.2081610
## ALK  0.2510943 0.3163156 0.2575325 1.0000000 0.3398582 0.1974687
## AVGO 0.4152308 0.3057328 0.3943849 0.3398582 1.0000000 0.1088351
## AWK  0.1361769 0.2657427 0.2081610 0.1974687 0.1088351 1.0000000
{% endhighlight %}
